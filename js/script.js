const products = [];
  ...
  function filterProducts() {
    ...
    return products.filter(p => {
      const okTerm = p.name.toLowerCase().includes(term) || p.barcode.includes(term);
  ...
  function renderPagination(totalItems) {
    ...
    const endPage = showEllipsis ? Math.min(totalPages - 1, startPage + maxVisiblePages - 2) : totalPages;
